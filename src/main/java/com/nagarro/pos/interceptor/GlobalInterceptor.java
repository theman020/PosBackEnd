...
/**
 * This class GlobalInterceptor authenticates requests
 * 
...
public class GlobalInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (request.getMethod().equalsIgnoreCase("OPTIONS")) {
			...
			String credential = "secret:client";
			String encodedString = Base64.getEncoder().encodeToString(credential.getBytes());

			String originalToken = "Basic " + encodedString;

			String receivedToken = request.getHeader("Authorization");

			System.out.println(originalToken.equals(receivedToken));
			if (originalToken.equals(receivedToken)) {
				return true;
			}
		}
